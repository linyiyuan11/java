...
> Appel 式回收
>
> - 把新时代分为一块较大的 Eden 空间和较小的两个 Survivor(s1,s2)空间。
> - 每次分配只是用 Eden 和其中一块 Survivor(s1)空间上，然后直接清理掉 Eden 和已用过的那块 Survivor(s1) 空间。
> - 下一次Java进行对象分配时就使用Eden和Survivor(s2)进行分配，如此进行循环
...
每次对所有的垃圾进行处理，会造成系统长时间的停顿。而增量算法的思想是垃圾收集线程每次只收集一小部分区域的内存空间，和用户线程交替执行，直到垃圾回收完毕。这样就可以减少每次系统停顿的时间了。但是缺点也很明显：频繁的切换上下文会影响系统的吞吐量。
...
![img](https://img-blog.csdnimg.cn/68ad8f521cbb47e4943db2abc6bdecae.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5pqC5YGc44CB566X5LiN566X5pS-5byD,size_20,color_FFFFFF,t_70,g_se,x_16)

​       在讨论分代收集理论的时候，采用了名为记忆集的数据结构来避免扫描整个老年代。记忆集是一种用于记录从非收集区域指向收集区域的指针集合的抽象数据结构。如果我们不考虑效率和成本的话，最简单的实现可以用非收集区域中所有含跨代引用的对象数组来实现这个数据结构。
...
一个卡页的内存中通常包含不止一个对象，只要卡页内有一个（或更多）对象的字段存在着跨代指针，那就将对应卡表的数组元素的值标识为 1，称为这个元素变脏（Dirty），没有则标识为 0。在垃圾收集发生时，只要筛选出卡表中变脏的元素，就能轻易得出哪些卡页内存块中包含跨代指针，把它们加入 GC Roots 中一并扫描。卡表通过写屏障和读屏障更新卡表。

![img](https://img-blog.csdnimg.cn/ac564b5077c54a8fade2c6c774fa4f11.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5pqC5YGc44CB566X5LiN566X5pS-5byD,size_14,color_FFFFFF,t_70,g_se,x_16)
...
新生代、伊甸园 (E) GC 后，幸存的对象会被复制到幸存区 (S) 中。另一些幸存者区 S 中不够年龄进不来 O 的也会被复制到 S 中。年龄够的会被复制到 O 中。老年代 GC 后一些幸存的对象会把 O 中的复制到 O 中。
